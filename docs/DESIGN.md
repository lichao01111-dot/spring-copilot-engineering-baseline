...
安全和性能不是发布后才处理的非功能项。风险相关变更必须完成 `security-performance-assessment`，由构建包装运行凭据扫描、安全高风险扫描和性能风险扫描。未批准的 `SEC-*` 高风险 Java 用法在本地和 CI 均阻断；`PERF-*` 风险在本地提示、在 CI 严格阻断。扫描只是提示风险，仍需威胁模型、性能预算、测试/压测证据和独立审查。详细规则见 [安全与性能完成门禁](../standards/security-and-performance-gates.md)。
